...
class Processing:
    ...
    def post_processing(self, result, image_shape, ori_image_shape):
        box = result['boxes']
        ...
        if 'masks' in result:
            mask = result['masks']
            mask = mask_to_image(mask, box, 1, ori_image_shape)
            result['masks'] = mask
...
def mask_to_image(mask, box, padding, image_shape):
